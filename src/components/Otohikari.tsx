...
export function Otohikari() {
  ...
  const testSpots = useMemo<Spot[] | null>(() => {
    ...
    for (let i = 0; i < Math.min(n, 50000); i++) {
      ...
      // 都市の重みが大きいほど広がりも大きく(都市圏)。海に落ちにくいようσは控えめ
      const sigma = 0.12 + Math.sqrt(city[2]) * 0.045;
      const lat = city[0] + gauss() * sigma;
      const lng = city[1] + (gauss() * sigma) / Math.max(0.2, Math.cos((city[0] * Math.PI) / 180));
