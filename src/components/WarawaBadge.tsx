 ...
 */
export function WarawaBadge({ size = 14, sir = false }: { size?: number; sir?: boolean }) {
  return (
    <svg
      width={size}
      height={size}
      ...
      role="img"
      aria-label={sir ? "Warawa-Sir（さとうみつろう）" : "わらわ〜会員（認証済み）"}
      className="inline-block flex-shrink-0 align-[-2px]"
    >
      <title>{sir ? "Warawa-Sir（さとうみつろう）" : "わらわ〜会員（認証済み）"}</title>
      {/* 波形の縁どり円（水色）。Sirだけ黒で囲む */}
      <path
        ...
        fill="#2CB7DE"
        stroke={sir ? "#141414" : "none"}
        strokeWidth={sir ? 2.2 : 0}
      />
