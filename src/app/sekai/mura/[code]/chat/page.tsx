...
/** セカイムラ◯◯県の全体チャット — TalKのグループ「セカイムラ◯◯」と完全同期(同じテーブル) */
export default function SekaiMuraChatPage() {
  ...
  // 村人がチャットを開いたら自動参加 → TalKのグループ欄に「セカイムラ◯◯」が現れる
  useEffect(() => {
    if (!me || !roomId || !murabito || joinedRef.current) return;
    joinedRef.current = true;
    createClient().from("pref_room_members").upsert({ room_id: roomId, user_id: me.id }).then(() => {});
  }, [me, roomId, murabito]);
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col lg:max-w-3xl" style={{ background: "#eef4ee", paddingBottom: kb > 0 ? kb + 64 : 120 }}>
      ...
      {gateChecked && me && !murabito && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/55 px-6">
          <div className="w-full max-w-[360px] rounded-2xl bg-white p-6 text-center">
            ...
            <Link href={`/sekai/mura/${idx}`} className="mt-2 block py-2 text-[12px] font-bold text-[#a09a88] no-underline">セカイムラ{disp}トップへ戻る</Link>
          </div>
        </div>
      )}
      ...
      <header className="sticky top-0 z-40 flex items-center gap-3 px-4 pb-3 pt-3.5" style={{ background: "linear-gradient(150deg,#2a7a48,#1e4530)" }}>
        <Link href={`/sekai/mura/${idx}`} className="text-[15px] font-bold text-[#cfe8d0] no-underline">◀</Link>
        <span className="text-[20px]">💬</span>
        <div className="min-w-0 flex-1">
          <div className="truncate text-[15px] font-bold leading-tight text-white">セカイムラ{disp} 全体チャット</div>
          {memberCount != null && <div className="num text-[10px] text-[#a8d4b0]">{memberCount}人 ・ TalKのグループと同期中</div>}
        </div>
