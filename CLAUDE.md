...
- **MMM** — キャッチ「夢とヒラメキの保管庫」。シューマン音は無料15秒試聴（再生位置ハードキャップ・時限式は聴き直し悪用があり却下済み）。OTOHIKARI地球儀: 雷=Blitzortung WSに**ユーザー端末が直接続**（うちの帯域ゼロ）、光柱=聴いてる人のpresence。**聴いた光は1人1点**: その日(3:36リセット)最初に位置が取れた再生場所が灯り、以後どこで聴いても増えない（listensのlat/lngをotohikari_snapshotでdistinct on集計・位置付きlistensが無い人はbeaconsで補完）。**TODAYはユニーク人数**（再生回数ではない）。beaconsは1人1行upsertなので**位置が取れない時はlat/lngを送らない**（nullで上書きすると既存の光が消える事故が実際に起きた）
